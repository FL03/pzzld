...
# Date: 2025-05-12
# Description: Dockerfile for pzzld
# License: Apache-2.0
...
COPY --from=builder --chown=auser:agroup /app/target/release/puzzler /usr/local/bin/puzzler
...
COPY --from=builder --chown=auser:agroup --chmod=755 --link /app/pzzld.toml* /opt/pzzld/.config/pzzld.toml*
# set the permissions
...
ENV APP_MODE=release \
    APP_HOST="0.0.0.0" \
    APP_PORT=8080 \
    APP_CONFIG_DIR="/opt/pzzld/.config" \
    RUST_LOG="pzzld=debug,info,hyper=info"
